...
function logMessage(message) {
    ...
    if (messageLog.length > 20) messageLog.pop();
    renderLog();
}
...
function showView(viewElement) {
    ...
    // Explicitly handle shop modal visibility based on gameState
    if (gameState === GAME_STATE.SHOP) {
        shopModal.classList.remove('hidden'); // Use class instead of style for consistency
        modalBackdrop.style.display = 'block'; // Backdrop can still use style
        if (!shopModal) console.error("ERREUR CRITIQUE: Élément shop-modal non trouvé!"); // Debug check
    } else {
...
// --- Rendering Functions ---
function renderAll() {
    try { // Wrap in try...catch to potentially catch rendering errors
        renderStats();
        renderLocationInfo();
        ...
        switch (gameState) {
            ...
            case GAME_STATE.SHOP:
                showView(null); // No main view, rely on modal logic in showView
                renderShop();
                break;
        ...
        // Ensure game over screen hidden if not game over
        if (gameState !== GAME_STATE.GAME_OVER) {
            gameOverScreen.classList.add('hidden');
        }
    } catch (error) {
        console.error("Erreur majeure dans renderAll:", error);
        logMessage("ERREUR: Une erreur est survenue lors du rendu du jeu. Vérifiez la console.");
        // Optionally try to recover or show an error message
    }
}
...
function formatAccessoryName(accessory) {
    ...
    if (!rarityInfo) return 'Accessoire Inconnu'; // Fallback
    // Return HTML string to be used with innerHTML
...
function renderStats() {
    if (!player) return; // Don't render if player doesn't exist yet
    ...
    if (statRing1) statRing1.innerHTML = formatAccessoryName(player.equipment.ring1); // Use innerHTML here
    if (statRing2) statRing2.innerHTML = formatAccessoryName(player.equipment.ring2); // Use innerHTML here
    if (statAmulet) statAmulet.innerHTML = formatAccessoryName(player.equipment.amulet); // Use innerHTML here

    const hpPercent = calculatedStats.maxHp > 0 ? Math.min(100, (player.hp / calculatedStats.maxHp) * 100) : 0;
    const mpPercent = calculatedStats.maxMp > 0 ? Math.min(100, (player.mp / calculatedStats.maxMp) * 100) : 0;
    let xpPercent = 0;
    if (nextLevelXP !== Infinity) {
        const xpForThisLevel = nextLevelXP - currentLevelXP;
        const xpGainedThisLevel = currentXP - currentLevelXP;
        xpPercent = xpForThisLevel > 0 ? Math.min(100, (xpGainedThisLevel / xpForThisLevel) * 100) : 100;
    } else { xpPercent = 100; }

    hpBar.style.width = `${hpPercent}%`; mpBar.style.width = `${mpPercent}%`; xpBar.style.width = `${xpPercent}%`;
    hpBar.textContent = `${player.hp}/${calculatedStats.maxHp}`; mpBar.textContent = `${player.mp}/${calculatedStats.maxMp}`;
    xpBar.textContent = nextLevelXP !== Infinity ? `${currentXP}/${nextLevelXP}` : 'MAX';
}

function renderLocationInfo() {
    let locationName = "Inconnu";
    if (!currentDungeon || !currentDungeon.id) locationName = "Nulle Part"; // Handle missing dungeon data
...
function renderLog() {
    if(messageLogElement) {
        ...
        console.error("ERREUR: Élément message-log non trouvé!");
    }
}

function renderDungeon() {
    let mapHTML = '';
    if (!currentDungeon || !currentDungeon.map || currentDungeon.map.length === 0) {
        mapDisplay.innerHTML = '<p style="color: white;">Erreur: Carte non chargée.</p>'; return;
    }
    ...
    for (let y = 0; y < mapHeight; y++) { for (let x = 0; x < mapWidth; x++) {
            const posKey = `${x},${y}`; let mapChar = currentDungeon.map[y]?.[x] || TILE.WALL; // Fallback if row undefined
            let tileClasses = ['tile']; let title = '';
            if (player && player.pos.x === x && player.pos.y === y) { tileClasses.push('tile-player'); title = "Vous"; }
            else if (currentDungeon.enemies?.[posKey]) {
                const enemyData = currentDungeon.enemies[posKey]; const enemyInfo = Enemies[enemyData.type];
                if(enemyInfo){ tileClasses.push('tile-enemy');
                    ...
                    else tileClasses.push('tile-enemy-g'); title = enemyInfo.name; }
                 else { tileClasses.push('tile-floor'); } // Error case: unknown enemy type
            } else if (currentDungeon.items?.[posKey]) {
                const item = currentDungeon.items[posKey];
                 if (item.type === 'chest') { tileClasses.push('tile-chest'); title = "Coffre"; }
                 else { tileClasses.push('tile-item');
                    if (item.type === 'gold') { tileClasses.push('tile-gold'); title = `${item.value} Or`; }
                    else if (item.type === 'potion') { tileClasses.push('tile-potion'); title = Items[item.itemId]?.name || "Potion"; }
                 }
            } else { switch (mapChar) {
                    ...
                    case TILE.HUB_DUNGEON_1: tileClasses.push('tile-hub', 'tile-hub-dungeon', 'dungeon-1'); title = Dungeons['dungeon1']?.name || "Donjon 1"; break;
                    case TILE.HUB_DUNGEON_2: tileClasses.push('tile-hub', 'tile-hub-dungeon', 'dungeon-2'); title = Dungeons['dungeon2']?.name || "Donjon 2"; break;
...
function renderCombat() {
    if (!combatState.active || !combatState.enemy) { combatDisplay.innerHTML = "<p>Erreur Combat.</p>"; return; }
...
function renderShop() {
    if (!player || !shopGoldDisplay || !shopContent) { // Add checks for elements
        console.error("Erreur dans renderShop: Joueur ou éléments DOM manquants.");
        return;
    }
    try { // Wrap in try-catch specifically for this complex function
        ...
        for (const categoryName in itemCategories) {
             if (!itemCategories.hasOwnProperty(categoryName)) continue;
            if (itemCategories[categoryName].length > 0 || categoryName === 'Vendre') {
                shopHTML += `<div class="shop-category"><h3>${categoryName}</h3>`;
                if (categoryName === 'Vendre') { shopHTML += '<p style="font-size: 0.9em; color: var(--text-medium);">Vendez votre équipement non utilisé (50% prix).</p>'; if (itemCategories['Vendre'].length === 0) shopHTML += "<p><i>Rien à vendre.</i></p>"; }
                itemCategories[categoryName].forEach(id => {
                    let itemData, cost, effectText, icon, actionButton; const isSelling = categoryName === 'Vendre';
                    let itemName = 'Objet Inconnu'; // Default name

                    if (isSelling) {
                        itemData = Items[id]; cost = Math.floor(itemData?.cost * 0.5 || 0);
                        effectText = `Quantité: ${player.inventory[id]}`; icon = itemData?.icon || '❓'; itemName = itemData?.name || '?';
                        ...
                        itemData = Skills[id]; cost = itemData?.cost || 0;
                        effectText = getSkillDescription(itemData) + (itemData?.type === 'active' ? ` (Coût: ${itemData?.mpCost}💧)` : ' (Passif)');
                        icon = itemData?.icon || '❓'; itemName = itemData?.name || '?'; const canAfford = player.gold >= cost;
                        actionButton = `<button class="shop-button modal-button" data-item-id="${id}" onclick="ShopManager.learnSkill('${id}')" ${!canAfford || cost <= 0 ? 'disabled' : ''}>Apprendre</button>`;
                    } else { // Buying Weapons, Armor, Consumables
                        itemData = Items[id]; cost = itemData?.cost || 0;
                        if (itemData?.type === 'weapon') effectText = `Att +${itemData.attack || 0}${itemData.mpBonus ? ', PM Max +'+itemData.mpBonus : ''}`;
                        else if (itemData?.type === 'armor') effectText = `Déf +${itemData.defense || 0}`;
                        else if (itemData?.type === 'potion') effectText = itemData.effect === 'heal' ? `Soigne ${itemData.value} PV` : `Rend ${itemData.value} PM`;
                        else effectText = '';
                        icon = itemData?.icon || '❓'; itemName = itemData?.name || '?'; const canAfford = player.gold >= cost;
                        actionButton = `<button class="shop-button modal-button" data-item-id="${id}" onclick="ShopManager.buyItem('${id}')" ${!canAfford || cost <= 0 ? 'disabled' : ''}>Acheter</button>`;
                    }
                    shopHTML += `<div class="shop-item"><span class="shop-item-icon">${icon}</span><div class="shop-item-info"><span class="shop-item-name">${itemName}</span><span class="shop-item-effect">${effectText}</span></div>${!isSelling ? `<span class="shop-item-cost">💰 ${cost}</span>` : ''}${actionButton}</div>`;
                }); shopHTML += `</div>`;
            }
        }
        shopContent.innerHTML = shopHTML;
    } catch (error) {
        console.error("Erreur dans renderShop:", error);
        shopContent.innerHTML = "<p style='color:red;'>Erreur lors du chargement de la boutique.</p>";
    }
}


function renderInventory() {
    if (!player || !inventoryDisplay) return;
    ...
    try {
        ...
        for (const itemId in player.inventory) { if(!player.inventory.hasOwnProperty(itemId)) continue; if (player.inventory[itemId] > 0 && Items[itemId]?.type === 'potion') { hasConsumables = true; const item = Items[itemId]; inventoryHTML += `<div class="list-item"><span>${item.icon || '?'} <strong>${item.name}</strong> x ${player.inventory[itemId]}</span><span> <button class="inventory-button" onclick="InventoryManager.usePotion('${itemId}')">Utiliser</button> </span></div>`; } }
        if (!hasConsumables) inventoryHTML += "<p><i>Aucun consommable.</i></p>";

        inventoryHTML += `<h3>Accessoires Non Équipés</h3>`;
        if (!player.accessoriesInventory || player.accessoriesInventory.length === 0) { inventoryHTML += "<p><i>Aucun accessoire trouvé.</i></p>"; }
        else { player.accessoriesInventory.forEach((acc, index) => { // Added index for safety
                if (!acc) { console.warn(`Accessoire null trouvé à l'index ${index}`); return; } // Skip null accessories
                const rarityInfo = RARITIES[acc.rarity]; if (!rarityInfo) return; // Skip if rarity invalid
                let statsHTML = '<em class="accessory-stats">'; let statsCount = 0;
                for (const stat in acc.stats) { if (!acc.stats.hasOwnProperty(stat)) continue; let statText = stat.replace('maxHp', 'PV+').replace('maxMp', 'PM+').replace('attack', 'Att+').replace('defense', 'Def+').replace('critChance', 'Crit%').replace('dodgeChance', 'Esq%'); statsHTML += `${statsCount > 0 ? ', ' : ''}${statText}${acc.stats[stat]}`; statsCount++; } statsHTML += '</em>';
                inventoryHTML += `<div class="list-item accessory-item"><span class="accessory-name" style="color:${rarityInfo.color};" title="${getAccessoryTooltip(acc)}">${acc.type === ACCESSORY_TYPE.RING ? '💍' : '📿'} <strong>${acc.name || 'Accessoire'}</strong> [${rarityInfo.name}]</span>${statsHTML}<button class="inventory-button equip-button" onclick="InventoryManager.equipAccessory('${acc.id}')">Équiper</button></div>`; });
        }
        inventoryHTML += '<hr><button class="modal-button" onclick="GameManager.closeInventory()">Fermer</button>';
    } catch (error) {
        console.error("Erreur dans renderInventory:", error);
        inventoryHTML = "<h2>Erreur Inventaire</h2><p>Impossible d'afficher l'inventaire.</p><hr><button class='modal-button' onclick='GameManager.closeInventory()'>Fermer</button>";
    }
    inventoryDisplay.innerHTML = inventoryHTML;

    // Update equipped names inside the inventory after content is set (safer)
...
function renderSkills() {
    if (!player || !skillsDisplay) return;
    let skillsHTML = `<h2>✨ Compétences</h2>`;
     try {
        if (!player.skills || player.skills.length === 0) { skillsHTML += "<p><i>Aucune compétence connue.</i></p>"; }
        else {
            let actives = player.skills.filter(id => Skills[id]?.type === 'active');
            let passives = player.skills.filter(id => Skills[id]?.type === 'passive');
            skillsHTML += "<h3>Actives</h3>";
            if (actives.length > 0) { actives.forEach(id => { const s = Skills[id]; skillsHTML += `<div class="list-item"><span>${s.icon || '?'} <strong>${s.name}</strong></span><em>${getSkillDescription(s)} (Coût: ${s.mpCost}💧)</em></div>`; }); }
            else skillsHTML += "<p><i>Aucune compétence active.</i></p>";
            skillsHTML += "<h3>Passives</h3>";
            if (passives.length > 0) { passives.forEach(id => { const s = Skills[id]; skillsHTML += `<div class="list-item"><span>${s.icon || '?'} <strong>${s.name}</strong></span><em>${getSkillDescription(s)}</em></div>`; }); }
            else skillsHTML += "<p><i>Aucune compétence passive.</i></p>";
        }
        skillsHTML += '<hr><button class="modal-button" onclick="GameManager.closeSkills()">Fermer</button>';
     } catch (error) {
         console.error("Erreur dans renderSkills:", error);
         skillsHTML = "<h2>Erreur Compétences</h2><p>Impossible d'afficher les compétences.</p><hr><button class='modal-button' onclick='GameManager.closeSkills()'>Fermer</button>";
     }
...
function getSkillDescription(skill) {
    ...
    switch(skill.effect) {
        case 'damage': return `Inflige ${skill.multiplier}x dégâts Attaque.`;
        case 'magic_damage': return `Inflige ${skill.baseDamage} dégâts magiques.`;
        case 'heal': return `Restaure ${skill.value} PV.`;
        case 'stat_boost':
            let statName = skill.stat || 'inconnue'; let suffix = '';
            if (statName === 'maxHp') statName = "PV Max"; else if (statName === 'maxMp') statName = "PM Max";
            else if (statName === 'attack') statName = "Attaque"; else if (statName === 'defense') statName = "Défense";
            else if (statName === 'critChance') { statName = "Critique"; suffix='%'; } else if (statName === 'dodgeChance') { statName = "Esquive"; suffix='%'; }
            return `Augmente ${statName} de ${skill.value}${suffix}.`;
        case 'defense_boost': return `Augmente Défense de ${skill.value} (1 tour).`;
        default: return "Effet non spécifié.";
    }
}

function calculatePlayerStats() {
    if (!player) return PLAYER_BASE_STATS; // Return base if player not initialized
    let combinedStats = { maxHp: PLAYER_BASE_STATS.hp + (player.levelBonus?.maxHp || 0), maxMp: PLAYER_BASE_STATS.mp + (player.levelBonus?.maxMp || 0), attack: PLAYER_BASE_STATS.attack + (player.levelBonus?.attack || 0), defense: PLAYER_BASE_STATS.defense + (player.levelBonus?.defense || 0), critChance: 0, dodgeChance: 0 };
    if (player.equipment?.weapon && Items[player.equipment.weapon]) { const w = Items[player.equipment.weapon]; combinedStats.attack += w.attack || 0; combinedStats.maxMp += w.mpBonus || 0; }
    if (player.equipment?.armor && Items[player.equipment.armor]) { combinedStats.defense += Items[player.equipment.armor].defense || 0; }
    player.skills?.forEach(skillId => { const skill = Skills[skillId]; if (skill?.type === 'passive' && skill.effect === 'stat_boost' && combinedStats.hasOwnProperty(skill.stat)) { combinedStats[skill.stat] += skill.value; } });
    ['ring1', 'ring2', 'amulet'].forEach(slot => { const accessory = player.equipment?.[slot]; if (accessory?.stats) { for (const statName in accessory.stats) { if (accessory.stats.hasOwnProperty(statName) && combinedStats.hasOwnProperty(statName)) { combinedStats[statName] += accessory.stats[statName]; } } } });
    combinedStats.maxHp = Math.max(1, combinedStats.maxHp); combinedStats.maxMp = Math.max(0, combinedStats.maxMp); combinedStats.attack = Math.max(0, combinedStats.attack); combinedStats.defense = Math.max(0, combinedStats.defense); combinedStats.critChance = Math.max(0, combinedStats.critChance); combinedStats.dodgeChance = Math.max(0, combinedStats.dodgeChance);
    return combinedStats;
}

// --- Dungeon Generation ---
// (generateSimpleDungeon function remains the same as previous correct version)
function generateSimpleDungeon(width, height, floor, maxEnemies, maxItems) {
    ...
    const dungeonData = Dungeons[currentDungeon?.id]; // Add check for currentDungeon
...
const AccessoryGenerator = {
    createAccessory: function(rarityKey) {
        const rarity = RARITIES[rarityKey]; if (!rarity) { console.error("Rareté invalide:", rarityKey); rarityKey = 'COMMON'; rarity = RARITIES.COMMON; }
        const accessory = { id: Date.now() + '-' + Math.random().toString(36).substring(2, 9), type: Math.random() < 0.6 ? ACCESSORY_TYPE.RING : ACCESSORY_TYPE.AMULET, rarity: rarityKey, name: '', stats: {} };
        let availableStats = [...POSSIBLE_ACCESSORY_STATS]; const numStats = Math.min(availableStats.length, rarity.maxStats);
        for (let i = 0; i < numStats; i++) { if (availableStats.length === 0) break; const statIndex = getRandomInt(0, availableStats.length - 1); const statName = availableStats.splice(statIndex, 1)[0]; let value = 0; let baseValue = 1;
            switch (statName) { case 'maxHp': baseValue = getRandomInt(5, 10); break; case 'maxMp': baseValue = getRandomInt(3, 7); break; case 'attack': baseValue = getRandomInt(1, 2); break; case 'defense': baseValue = getRandomInt(1, 2); break; case 'critChance': baseValue = getRandomInt(1, 3); break; case 'dodgeChance': baseValue = getRandomInt(1, 3); break; }
            value = Math.max(1, Math.round(baseValue * rarity.statMultiplier * (0.8 + Math.random() * 0.4))); accessory.stats[statName] = value; }
        accessory.name = `${accessory.type} ${this.generateNameSuffix(accessory.stats)}`; return accessory;
    },
    generateNameSuffix: function(stats) { let bestStat = ''; let bestValue = -1; for (const stat in stats) { if(!stats.hasOwnProperty(stat)) continue; let currentWeight = 1; switch (stat) { case 'maxHp': currentWeight = 0.2; break; case 'maxMp': currentWeight = 0.3; break; case 'attack': currentWeight = 1.5; break; case 'defense': currentWeight = 1.2; break; } let weightedValue = stats[stat] * currentWeight; if (weightedValue > bestValue) { bestValue = weightedValue; bestStat = stat; } }
        switch (bestStat) { case 'maxHp': return "de Vitalité"; case 'maxMp': return "d'Esprit"; case 'attack': return "de Force"; case 'defense': return "de Garde"; case 'critChance': return "de Précision"; case 'dodgeChance': return "d'Agilité"; default: return "Mystérieux"; }
    }
};
...
const GameManager = {
    ...
    checkLevelUp: function() { if (!player) return; const xpNeeded = LEVEL_XP_REQUIREMENT[player.level]; if (xpNeeded && player.xp >= xpNeeded && player.level < LEVEL_XP_REQUIREMENT.length -1 ) { const xpKept = player.xp - xpNeeded; player.level++; player.xp = xpKept; logMessage(`⭐ Niveau ${player.level} ! ⭐`); const hpGain=getRandomInt(3,6); const mpGain=getRandomInt(1,3); const attackGain=getRandomInt(1,2); const defenseGain=getRandomInt(0,1); player.levelBonus.maxHp += hpGain; player.levelBonus.maxMp += mpGain; player.levelBonus.attack += attackGain; player.levelBonus.defense += defenseGain; const stats=calculatePlayerStats(); player.hp = Math.min(player.hp + Math.ceil(stats.maxHp*0.25), stats.maxHp); player.mp = Math.min(player.mp + Math.ceil(stats.maxMp*0.25), stats.maxMp); logMessage(`Stats + | PV/PM restaurés.`); this.saveGame(); this.checkLevelUp(); renderAll(); } }, // Recursive call removed, let gainXP call it if needed
    ...
    triggerDamageFlash: function() { if(playerStatsPanel) { playerStatsPanel.classList.add('flash-damage'); setTimeout(() => { playerStatsPanel.classList.remove('flash-damage'); }, 400); } },
...
const InventoryManager = {
    ...
    equipAccessory: function(accessoryId) { const accIndex = player.accessoriesInventory?.findIndex(acc => acc && acc.id === accessoryId); if (accIndex === -1 || accIndex === undefined) { logMessage("Accessoire introuvable."); return; } const accessory = player.accessoriesInventory[accIndex]; let targetSlot = null; if (accessory.type === ACCESSORY_TYPE.RING) { if (!player.equipment.ring1) targetSlot = 'ring1'; else if (!player.equipment.ring2) targetSlot = 'ring2'; else { logMessage("Anneaux pleins."); return; } } else if (accessory.type === ACCESSORY_TYPE.AMULET) { if (!player.equipment.amulet) targetSlot = 'amulet'; else { logMessage("Amulette pleine."); return; } } else { logMessage("Type inconnu."); return; } const currentlyEquipped = player.equipment[targetSlot]; if(currentlyEquipped) { player.accessoriesInventory.push(currentlyEquipped); /* logMessage(`Déséquipement auto: ${formatAccessoryName(currentlyEquipped)}.`); */ } player.accessoriesInventory.splice(accIndex, 1); player.equipment[targetSlot] = accessory; const rarityInfo = RARITIES[accessory.rarity]; logMessage(`Équipé: <span style="color:${rarityInfo?.color || '#fff'};">[${rarityInfo?.name || '?'}] ${accessory.name}</span>.`); renderAll(); GameManager.saveGame(); },
...
// --- Initialize Game ---
// Ensure the DOM is ready before initializing (though script at end of body helps)
