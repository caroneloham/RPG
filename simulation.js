...
const BuildingTypes = {
    ...
    // Ajoutez d'autres bâtiments ici pour atteindre 15 ou plus si désiré
};
    ...
    calculateBuildingEffect: function(buildingType, level) {
        ...
        if (buildingType.baseEffect.productionRate) {
         ...
         if (buildingType.baseEffect.heroCapacity) {
            ...
            effect.heroCapacity = buildingType.baseEffect.heroCapacity * level; // Ou juste +1 par niveau si mult=1
         }
    ...
    spendResources: function(cost) {
        for (const resource in cost) {
            if (this.resources[resource] < cost[resource]) {
                const message = `Pas assez de ${resource}. Requis: ${cost[resource]}, Disponible: ${Math.floor(this.resources[resource])}`;
    ...
    addResources: function(gains) {
     ...
     buildBuilding: function(buildingTypeId) {
         ...
         if (this.constructedBuildings.some(b => b.typeId === buildingTypeId)) {
             const message = `Vous avez déjà construit un(e) ${type.name}.`;
             console.warn(message);
              if (typeof UI !== 'undefined' && UI.displayMessage) {
         ...
         // Vérifier le coût
         const cost = this.calculateCost(type, 0); // Coût niveau 1
         if (this.spendResources(cost)) {
             ...
             if (typeof UI !== 'undefined') {
                 UI.updateBuildModeUI(); // Rafraîchir la liste des bâtiments
     ...
     upgradeBuilding: function(buildingTypeId) {
         ...
         const cost = this.calculateCost(type, building.level); // Coût pour passer au niveau suivant
         if (this.spendResources(cost)) {
             // Augmenter le niveau
             building.level++;
             console.log(`${type.name} amélioré au niveau ${building.level} !`);

             // Recalculer tous les effets
             this.recalculateBuildingEffects();

             // Mettre à jour l'UI
             if (typeof UI !== 'undefined') {
                 UI.updateBuildModeUI(); // Rafraîchir la liste des bâtiments (coût/niveau)
                 UI.drawGame(); // Redessiner le canvas (potentiellement changer l'apparence)
      ...
      getConstructedBuildingsData: function() {
          return this.constructedBuildings.map(b => {
              ...
              return {
                  ...b, // level, position, currentEffect
                  name: type.name,
                  maxLevel: type.maxLevel,
                  color: type.color, // Pour le dessin
                   upgradeCost: (b.level < type.maxLevel) ? this.calculateCost(type, b.level) : null
              };
          });
      }
};

// Initial recalculate after buildings are defined
// Simulation.recalculateBuildingEffects(); // Fait dans App.init maintenant